...
interface CreateUserInput { login: string, senha: string, cargo: "admin" | "funcionario", permissoes: PermissoesUsuario}
...
export async function criarUsuarioAction(input: CreateUserInput) {
  ...
  const supabase = await createClient(supabase_url, service_role_key, {
      auth: {
  ...
  try {
    ...
    .from("profiles").update({ cargo, }).eq("id", userId)
